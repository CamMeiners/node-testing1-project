...
function trimProperties(obj) {
  ...
  Object.keys(trimmedObject).forEach((property) => {
...
function findLargestInteger(integers) {
  const newInteger = integers.map((integer) => {
    return integer.integer;
  });
  return Math.max(...newInteger);
}
...
class Counter {
   ...
   */
  countDown() {
    const nummy = this.initialNumber;
...
class Car {
   ...
   */
  drive(distance) {
    const gasLikeItsJiffyLube = this.tank * this.mpg;
    if (gasLikeItsJiffyLube >= distance) {
      this.odometer += distance;
      const bruh = distance / this.mpg;
      this.tank = Math.floor(this.tank - bruh);
    } else {
      this.tank = 0;
      this.odometer += gasLikeItsJiffyLube;
    }
  }
